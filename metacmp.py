...
if __name__ == '__main__':
    ...
    if '-h' in myargs or len(myargs) == 0:  # Example usage.
        ...
        print('\t-l: Intermediate log directory')
    ...
    sample_name = myargs['-l'] + '/' + myargs['-c'].split('.')[0].split('/')[1]
    ...
    if not os.path.exists(os.getcwd()+"/"+acc_name):
        ...
        subprocess.call(["ncbi-blast/bin/blastn", "-db", os.path.dirname(os.path.abspath(__file__))+"/BlastDB/aclame", "-query", myargs['-c'], \
    ...
    if not os.path.exists(os.getcwd()+"/"+card_name):
        ...
        subprocess.call(["ncbi-blast/bin/blastx", "-db", os.path.dirname(os.path.abspath(__file__))+"/BlastDB/CARD_PROT", "-query", myargs['-g'], \
    ...
    if not os.path.exists(os.getcwd()+"/"+patric_name):
        ...
        subprocess.call(["ncbi-blast/bin/blastn", "-db", os.path.dirname(os.path.abspath(__file__))+"/BlastDB/PATRIC", "-query", myargs['-c'], \
    ...
    if not os.path.getsize(os.getcwd()+"/"+ card_name) > 0:
        ...
        result_name = sample_name + "_Results.txt"
        with open(result_name, 'w') as f:
            f.write("nContigs " + str(nContigs) + "\nnARG " + str(nARG) + "\nnMGE " + str(nMGE) + "\nnPAT " + str(nPAT) + "\nnARG&MGE " + str(nARG_MGE) + "\nnARG&MGE&PAT " + str(nARG_MGE_PAT) + "\nnARG/nContigs " + str(fARG) + "\nnMGE/nContigs " + str(fMGE) + "\nnPAT/nContigs " + str(fPAT) + "\nnARG&MGE/nContigs " + str(fARG_MGE) + "\nnARG&MGE&PAT/nContigs " + str(fARG_MGE_PAT) + "\n\nRisk Score " + str(score))
